...
final class DecodeHandler extends Handler {
	...
	public void handleMessage(Message message) {
		...
		switch (message.what) {
		case R.id.id_zxing_decode:
			decode((byte[]) message.obj, message.arg1, message.arg2);
			break;
		case R.id.id_zxing_quit:
			running = false;
		...
		if (rawResult != null) {
			...
			if (handler != null) {
				Message message = Message.obtain(handler,
						R.id.id_zxing_decode_succeeded, rawResult);
			...
			if (handler != null) {
				Message message = Message.obtain(handler, R.id.id_zxing_decode_failed);
				message.sendToTarget();
			}
		}
	}
